...
int invertList(headNode* h) {
	...
	listNode* prev = NULL;  //이전 노드
	listNode* mid = NULL;  //중간 노드

	while(now != NULL) {
		prev = mid;  //이전 노드를 임시 노드로
		mid = now;  //임시 노드를 현재 노드로
		now = now->rlink;  //다음 노드로 이동
		mid->rlink = prev;  //임시 노드(현재 노드였던 것)의 rlink를 이전 노드로
		mid->llink = now;  //임시 노드의 llink를 현재 노드로
	}

	h->first = mid;  //헤드노드가 가리키는 첫번째 노드를 mid 노드로 설정
